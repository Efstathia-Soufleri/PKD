...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train(train_loader, teacher_model_iframe, model_IC, criterion, optimizer_IC1, optimizer_IC2,
        ...
        if is_best_IC1:
            save_checkpoint(
                ...
                is_best_IC1,
                filename='checkpoint_KD_iframe_IC1_seed_'+str(args.random_seed)+'.pth.tar')
        ...
        if is_best_IC2:
            save_checkpoint(
                ...
                is_best_IC2,
                filename='checkpoint_KD_iframe_IC2_seed_'+str(args.random_seed)+'.pth.tar')
        ...
        if is_best_IC3:
            save_checkpoint(
                ...
                is_best_IC3,
                filename='checkpoint_KD_iframe_IC3_seed_'+str(args.random_seed)+'.pth.tar')
...
def train(train_loader, teacher_model_iframe, model_IC, criterion, optimizer_IC1, optimizer_IC2,
              optimizer_IC3, epoch, cur_lr1, cur_lr2, cur_lr3):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        teacher_out_iframe = teacher_model_iframe(input_i)
        teacher_out_iframe = teacher_out_iframe.view((-1, args.num_segments) + teacher_out_iframe.size()[1:])
        teacher_out_iframe = torch.mean(teacher_out_iframe, dim=1)

        loss1 = cross_entropy(softmax1(out_IC1/args.T), softmax1((teacher_out_iframe/args.T).detach()))
        loss2 = cross_entropy(softmax1(out_IC2/args.T), softmax1((teacher_out_iframe/args.T).detach()))
        loss3 = cross_entropy(softmax1(out_IC3/args.T), softmax1((teacher_out_iframe/args.T).detach()))
