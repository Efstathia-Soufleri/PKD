...
def main():
    ...
    model = Model(num_classes_kin, args.num_segments, args.representation,
                  base_model=args.arch)
    # print(model)

    # checkpoint = torch.load('./checkpoints/resnet18_residual__rolled_model__8f_checkpoint.pth.tar')
    print("Loaded the pretrained on kinetics model.")

    model.load_state_dict(torch.load('./checkpoints/resnet18_residual__rolled_model_v2__8f_checkpoint_v2.pth.tar')['state_dict'])
    # model.load_state_dict(torch.load('./checkpoints/resnet18_residual__rolled_model__8f_checkpoint.pth.tar')['state_dict'])
    # base_dict = {'.'.join(k.split('.')[1:]): v for k,v in list(checkpoint['state_dict'].items())}
    # model.base_model.load_state_dict(base_dict)

    feature_dim = getattr(model.base_model, 'fc').in_features
    setattr(model.base_model, 'fc', torch.nn.Linear(feature_dim, num_class))
    ...
    if args.optimizer == 'adam':
        ...
        optimizer = torch.optim.SGD(params, momentum=0.9, nesterov=True, weight_decay=0.00001)
    else:
        print("Provide the optimizer you want to use.")
