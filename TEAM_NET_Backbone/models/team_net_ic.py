...
class TEAM_IC(nn.Module):
    ...
    def forward(self, i_x_input, mv_x_input, r_x_input):
        ...
        return zs_i_out, zs_mv_out, zs_r_out, zc_squeeze
...
class TEAM_Net(nn.Module):
    def __init__(self, num_class, num_segments, 
        ...
        self.IC1_classifier = nn.Linear(384, num_class)
        self.IC2_classifier = nn.Linear(8 * 96, num_class) 
        self.IC3_classifier = nn.Linear(1536, num_class) # nn.Sequential(nn.Linear(1536, 512), nn.ReLU(), nn.Linear(512, num_class))

        self._ratio = 4
        if is_shift:
            self.team0 = TEAM_IC(self.MV_model.base_model.conv1.out_channels, 
                                        self.I_model.base_model.conv1.out_channels,
                                        ratio = self._ratio, num_segments = num_segments)  
            self.team1 = TEAM_IC(self.MV_model.base_model.layer2[0].conv1.net.in_channels, 
                                        self.I_model.base_model.layer2[0].conv1.net.in_channels,
                                        ratio = self._ratio, num_segments = num_segments)
            self.team2 = TEAM_IC(self.MV_model.base_model.layer3[0].conv1.net.in_channels, 
                                        self.I_model.base_model.layer3[0].conv1.net.in_channels,
                                        ratio = self._ratio, num_segments = num_segments)
            self.team3 = TEAM_IC(self.MV_model.base_model.layer4[0].conv1.net.in_channels, 
            ...
            self.team0 = TEAM_IC(self.MV_model.base_model.conv1.out_channels, 
                                        self.I_model.base_model.conv1.out_channels,
                                        ratio = self._ratio, num_segments = num_segments)  
            self.team1 = TEAM_IC(self.MV_model.base_model.layer2[0].conv1.in_channels, 
                                        self.I_model.base_model.layer2[0].conv1.in_channels,
                                        ratio = self._ratio, num_segments = num_segments)
            self.team2 = TEAM_IC(self.MV_model.base_model.layer3[0].conv1.in_channels, 
                                        self.I_model.base_model.layer3[0].conv1.in_channels,
                                        ratio = self._ratio, num_segments = num_segments)
            self.team3 = TEAM_IC(self.MV_model.base_model.layer4[0].conv1.in_channels, 
                                        self.I_model.base_model.layer4[0].conv1.in_channels,
                                        ratio = self._ratio, num_segments = num_segments) 

    def exit1(self, i_x, mv_x, r_x):
        bs = i_x.shape[0]
        ...
        # laterial 0
        i_x, mv_x, r_x, combined_l1 = self.team0(i_x, mv_x, r_x)
        combined_l1 = combined_l1.view(bs, -1)
        out_IC1 = self.IC1_classifier(combined_l1)

        return i_x, mv_x, r_x, combined_l1, out_IC1
    
    def exit2(self, i_x, mv_x, r_x):
        bs = i_x.shape[0]
        i_x, mv_x, r_x, combined_l1, out_IC1 = self.exit1(i_x, mv_x, r_x)
        # layer1
        ...
        # laterial 1
        i_x_res1, mv_x_res1, r_x_res1, combined_l2 = self.team1(i_x_res1, mv_x_res1, r_x_res1)
        combined_l2 = combined_l2.view(bs, -1)
        out_IC2 = self.IC2_classifier(combined_l2)

        return i_x_res1, mv_x_res1, r_x_res1, combined_l2, out_IC2
    
    def exit3(self, i_x, mv_x, r_x):
        bs = i_x.shape[0]
        i_x_res1, mv_x_res1, r_x_res1, combined_l1, out_IC1 = self.exit2(i_x, mv_x, r_x)
        # layer2
        ...
        # laterial 2
        i_x_res2, mv_x_res2, r_x_res2, combined_l3 = self.team2(i_x_res2, mv_x_res2, r_x_res2)
        combined_l3 = combined_l3.view(bs, -1)
        out_IC3 = self.IC3_classifier(combined_l3)
        return out_IC3
    ...
    def forward(self, i_x, mv_x, r_x):
        ...
        # laterial 3
        i_x_res3, mv_x_res3, r_x_res3, combined_l4 = self.team3(i_x_res3, mv_x_res3, r_x_res3)
        ...
        return out_IC1, out_IC2, out_IC3, out, i_x_pred, mv_x_pred, r_x_pred
