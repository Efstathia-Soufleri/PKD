...
class TEAM_Net(nn.Module):
    ...
    def forward(self, i_x, mv_x, r_x):
        ...
        # print(f"TEAm NEt 0{i_x_pool.shape}")

        i_x_pred = self.I_model.new_fc(i_x_pool)
        mv_x_pred = self.MV_model.new_fc(mv_x_pool)
        r_x_pred = self.R_model.new_fc(r_x_pool)

        # print(f"TEAm NEt {i_x_pred.shape} {r_x_pred.shape}  {mv_x_pred.shape}  {self._num_segments}")
        ...
        # print(f"TEAm NEt 4 {i_x_pred.shape} {r_x_pred.shape}  {mv_x_pred.shape}")
