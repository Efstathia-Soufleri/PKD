            ...
            if epoch % args.eval_freq == 0 or epoch == args.epochs - 1:
                ...
                if is_best_IC1:
                    save_checkpoint(
                        ...
                        is_best_IC1,
                        filename='checkpoint_KD_progressive_anti_IC1_seed_'+str(args.random_seed)+'_split_'+str(args.split)+'.pth.tar')
                ...
                if is_best_IC2:
                    save_checkpoint(
                        ...
                        is_best_IC2,
                        filename='checkpoint_KD_progressive_anti_IC2_seed_'+str(args.random_seed)+'_split_'+str(args.split)+'.pth.tar')
                ...
                if is_best_IC3:
                    save_checkpoint(
                        ...
                        is_best_IC3,
                        filename='checkpoint_KD_progressive_anti_IC3_seed_'+str(args.random_seed)+'_split_'+str(args.split)+'.pth.tar')
...
def train(train_loader, model, criterion, optimizers, epoch, cur_lr, writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if epoch < 15:
            loss1 = cross_entropy(softmax1(out_IC1/args.T), softmax1((teacher_out_i/args.T).detach()))
            loss2 = cross_entropy(softmax1(out_IC2/args.T), softmax1((teacher_out_i/args.T).detach()))
            loss3 = cross_entropy(softmax1(out_IC3/args.T), softmax1((teacher_out_i/args.T).detach()))
...
def save_checkpoint(state, is_best, filename):
    ...
    if is_best:
        best_name = f'{args.arch}_tsn_{args.num_segments}_TEAM_NET_{args.split}_seed_{args.random_seed}_Prog_KD_anti_IC_.pth.tar'
