...
class CoviarDataSet_inference(data.Dataset):
    ...
    def _load_list_ucf101(self, video_list):
        file_name = os.path.split(video_list)[-1].split('.')[0]
        save_path = os.path.join("./data", file_name + '.pkl')
        if os.path.exists(save_path):
            with open(save_path, 'rb') as f:
                self._video_list = pickle.load(f)
        else:
            self._video_list = []
            ...
            with open(save_path, 'wb') as f:
                pickle.dump(self._video_list, f)

        print('%d videos loaded.' % len(self._video_list))

    def _load_list_ucf101_old(self, video_list):
    # def _load_list_ucf101(self, video_list):
        self._video_list = []
        f = pd.read_csv(video_list, header=None)
        f = f.iloc[1:]
    ...
    def _load_list_kinetic400(self, video_list):
        self._video_list = []
        # print(video_list)
        if 'train' in video_list:
            with open("video_list_train.pkl", "rb") as pickle_file:
                self._video_list = pickle.load(pickle_file)
        else:
            with open("video_list_val.pkl", "rb") as pickle_file:
                self._video_list = pickle.load(pickle_file)
...
class CoviarDataSet(data.Dataset):
    ...
    def _load_list_kinetic400(self, video_list):
        ...
        f = pd.read_csv(video_list, header=None)
        f = f.iloc[1:]
        f_cs = pd.read_csv(video_list[:-4]+'.csv')
        ...
        # class_names = np.load('name_to_label.npy', allow_pickle=True)
        with open("name_to_label.pkl", "rb") as pickle_file:
            class_names = pickle.load(pickle_file)

        for i in trange(len(f_cs)):
            youtube_id = f_cs['youtube_id'].iloc[i]
            time_start = f_cs['time_start'].iloc[i]
            time_end = f_cs['time_end'].iloc[i]
            x = time_start
            x = '{:06}'.format(x)
            y = time_end
            y = '{:06}'.format(y)
            video_path_name = os.path.join(self._data_root,str(youtube_id)+'_'+x+'_'+y+'.mp4')

            # self._video_list.append((f_cs.iloc[i][0],  f_cs.iloc[i][0], f_cs.iloc[i][2]))
            # print(video_path_name)
            self._video_list.append((video_path_name,  class_names[f_cs.iloc[i][0]],get_num_frames(video_path_name)))
            max_val = max(max_val, self._video_list[i][2])
            min_val = min(min_val, self._video_list[i][2])
            all_len += self._video_list[i][2]
        print('{} good videos loaded!'.format(len(self._video_list)))
